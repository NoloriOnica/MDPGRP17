...
public class BluetoothPopUp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(mBroadcastReceiver3, discoverFilter);
        ...
        bluetoothSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                // check for modern Bluetooth permissions first
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    if (ActivityCompat.checkSelfPermission(BluetoothPopUp.this, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
                        // If no permission, request it and flip the switch back for now
                        checkBTPermissions();
                        compoundButton.setChecked(!isChecked);
                        return;
                    }
                }
                // only runs if permissions are granted)
                if(isChecked){
                ...
                if(mBluetoothAdapter == null){
                    Toast.makeText(BluetoothPopUp.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
    ...
    public void Scanning(){
        ...
        if(mBluetoothAdapter != null) {
            if (!mBluetoothAdapter.isEnabled()) {
                ...
                return; // Gracefully stop if Bluetooth is off
            }

            // 1. The Security Check (Banishes the red lines!)
            if (!hasBTPermissions()) {
                checkBTPermissions();
                Toast.makeText(this, "Please grant permissions and try scanning again.", Toast.LENGTH_SHORT).show();
                return; // Stop the code here until they grant the permission
            }

            // 2. The Crash Shield: Stop any ongoing scout before sending new ones
            if (mBluetoothAdapter.isDiscovering()) {
                mBluetoothAdapter.cancelDiscovery();
            }

            // 3. Start the fresh search safely
            Log.d(TAG, "Scanning: Starting fresh discovery.");
            mBluetoothAdapter.startDiscovery();

            // 4. Gather the paired devices safely
    ...
    private void checkBTPermissions() {
        ...
        // 1. Location Permissions (Required for scanning on all versions)
        permissionList.add(Manifest.permission.ACCESS_FINE_LOCATION);
        permissionList.add(Manifest.permission.ACCESS_COARSE_LOCATION);

        // 2. Bluetooth Specific Permissions (Required for API 31 / Android 12+)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
        ...
        // Convert list to array for the request
        ...
        if (!listPermissionsNeeded.isEmpty()) {
            ActivityCompat.requestPermissions(this,
                    listPermissionsNeeded.toArray(new String[0]), 1001);
        }
    }

    // A quick shield to check if we hold the keys to the Bluetooth permissions
    ...
    private final BroadcastReceiver mBroadcastReceiver5 = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(status.equals("connected")){
                MainActivity.isExpectedDisconnect = false;
                try {
                    myDialog.dismiss();
                ...
                // We force the app to wait exactly half a second before deciding what to do.
                // This gives MainActivity the time it needs to hear "STOP" and raise the shield!
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {

                        if (MainActivity.isExpectedDisconnect) {
                            Log.d(TAG, "Task complete. Graceful disconnect recognized.");
                            ...
                            // The shield is truly down. This was an accident!
                            Log.d(TAG, "mBroadcastReceiver5: Unexpectedly Disconnected from "+mDevice.getName());
                            ...
                            try {
                                ...
                                Log.d(TAG, "BluetoothPopUp: Dialog show failure");
                            }
