...
import com.example.MDP_G17.main.BluetoothChatFragment;
import com.example.MDP_G17.main.BluetoothConnectionService;
import com.example.MDP_G17.main.BluetoothPopUp;
import com.example.MDP_G17.main.ControlFragment;
import com.example.MDP_G17.main.GridMap;
import com.example.MDP_G17.main.MapTabFragment;
import com.example.MDP_G17.main.SectionsPagerAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    // A shield to prevent unwanted reconnections when the RPi finishes its task
    public static boolean isExpectedDisconnect = false;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_ACL_CONNECTED.equals(action)) {
                ...
                if (robotStatusText != null) {
                    robotStatusText.setText("Ready");
                    Toast.makeText(MainActivity.this, "Connected to Raspberry Pi", Toast.LENGTH_SHORT).show();
                }
            } else if (BluetoothDevice.ACTION_ACL_DISCONNECTED.equals(action)) {
                // The connection was lost or dropped
                if (robotStatusText != null) {
                    robotStatusText.setText("Not Connected");
    ...
    /**
     * Adds a new line after the message is sent, so that the next message will appear on new line
     ...
     * Message format:
     * TARGET-[obstacle id]-[target id] for image rec
     *  ex: TARGET-3-7 for obstacle 3 === target id 7
     * ROBOT-[x-coord]-[y-coord]-[Direction] - for updating robot coordinates
     *  ex: ROBOT-6-7-N (to put robot at coords (6,7) facing North
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (message.contains("TARGET")) {
                ...
                if (cmd.length == 3) {
                    runOnUiThread(() -> {
                        if (gridMap != null) {
                            gridMap.updateImageID(cmd[1].trim(), cmd[2].trim());
                            gridMap.invalidate();
                        }
                    });
                }
                ...
                String dirInput = cmd[3].trim().toUpperCase();

                String newDirection = "up";
                switch (dirInput) {
                    case "N": newDirection = "up"; break;
                    case "S": newDirection = "down"; break;
                    case "E": newDirection = "right"; break;
                    case "W": newDirection = "left"; break;
                }

                int[] newCoord = new int[]{xPos, yPos};
                String finalNewDirection = newDirection;

                runOnUiThread(() -> {
                    if (gridMap != null) {
                        gridMap.setCurCoord(newCoord);
                        gridMap.setRobotDirection(finalNewDirection);
                        gridMap.invalidate();
                        refreshCoordinate();
                    }
                });
                ...
                if (imgRecBtn != null && imgRecBtn.isChecked()) {
                    imgRecTimerFlag = true;
                    imgRecBtn.setChecked(false);
                    if (robotStatusText != null) robotStatusText.setText(R.string.image_rec_end);
                    if (controlFragment != null) {
                        ControlFragment.timerHandler.removeCallbacks(controlFragment.imgRecTimer);
                    }
                } else if (fastestCarBtn != null && fastestCarBtn.isChecked()) {
                    imgRecTimerFlag = true;
                    fastestCarBtn.setChecked(false);
                    if (robotStatusText != null) robotStatusText.setText(R.string.fastest_car_end);
                    if (controlFragment != null) {
