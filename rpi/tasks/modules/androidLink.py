from tasks.modules.android import AndroidMessage, AndroidLink
from multiprocessing import Process, Queue
import time

def continuous_recv(connection: AndroidLink, q: Queue):
...
def continuous_send(connection: AndroidLink, q: Queue):
    """Continuously read AndroidMessage fsrom q and send to Android."""
    while True:
        try:
...
def monitor_connection(connection: AndroidLink, procs: list, recv_q: Queue, send_q:Queue):
    ...
    while True:
        if not connection.get_connection_status():
            ...
            recv_p = Process(target=continuous_recv, args=(connection,recv_q))
            send_p = Process(target=continuous_send, args=(connection,send_q))
            recv_p.start()
            send_p.start()
