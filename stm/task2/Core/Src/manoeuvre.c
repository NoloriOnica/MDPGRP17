...
#define STRAIGHT_STEER_TRIM     -0.38f
...
static const char* CmdName(CmdType_t type)
{
    switch (type)
    {
        ...
        case CMD_FIRR:       return "FIRR";
        case CMD_FIRL:       return "FIRL";
        default:             return "NONE";
    }
}
...
static void Parse_Command(const char *line)
{
    ...
    else if (strcmp(cmd_str, "FIRR") == 0 && !has_param)
    {
        cmd.type  = CMD_FIRR;
    }
    else if (strcmp(cmd_str, "FIRL") == 0 && !has_param)
    {
        cmd.type  = CMD_FIRL;
    }
    ...
    {
        if (cmd.type == CMD_FIRR || cmd.type == CMD_FIRL || cmd.type == CMD_STOP)
            printf("ACK %s\r\n", cmd_str);
        else
            printf("ACK %s %.1f\r\n", cmd_str, cmd.param);
    }
...
static void Finish_Manoeuvre(const char *status)
{
    ...
    if (Servo_GetTick() < SERVO_TICK_CENTER) {
        Servo_SetTick(SERVO_TICK_CENTER + 3);
    } else {
        Servo_SetTick(SERVO_TICK_CENTER - 2);
    }
    ...
    if (current_cmd.type == CMD_APPROACH ||
        current_cmd.type == CMD_FIRR ||
        current_cmd.type == CMD_FIRL)
...
static void Start_ForwardIR(Command_t *cmd)
{
    (void)cmd;
...
static void Execute_Drive(void)
{
    ...
    if (fabsf(current_yaw) > STRAIGHT_YAW_DEADZONE)
    {
         ...
         */
        float sign = (current_cmd.type == CMD_BACKWARD) ? 1.0f : -1.0f;
    ...
    int16_t motor_speed;
    if (current_cmd.type == CMD_BACKWARD)
...
static void Execute_ForwardIR(void)
{
    IR_Sensor_ID_t sensor = (current_cmd.type == CMD_FIRR) ? IR_RIGHT : IR_LEFT;
    float ir_distance_cm = IR_Sensor_GetDistance_cm(sensor);

    if (ir_distance_cm == -1.0f)
    {
        Finish_Manoeuvre("DONE");
        return;
    }

    /* Reuse the standard straight-drive controller until the IR sensor clears. */
    current_cmd.param = drive_total_distance + 1000000.0f;
    Execute_Drive();
}
